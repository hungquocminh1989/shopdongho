					...
					if($listDetail != NULL && count($listDetail) > 0){
						...
						foreach($listDetail as $item){
							...
							if($section_type ==  SYSTEM_META_SECTION_CATEGORY){
								$data = $PageHandle->selectPage_CategoryData($m_site_page_section_id, $page_type);
								$this->appendData($data);
							}
							else if($section_type ==  SYSTEM_META_SECTION_PRODUCT){
								$m_group_data_id = $item['m_group_data_id'];
								$data = $PageHandle->selectPage_ProductData($m_site_page_section_id, $page_type);
								$this->appendData($data);
							}
								...
								$data = $PageHandle->selectPage_FreeHtmlData($m_site_page_section_id, $page_type);
								$this->appendData($data);
							}
